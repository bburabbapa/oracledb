...
--관계형데이터베이스
select employee_id, emp_name, a.department_id, b.department_name, b.parent_id, b.manager_id, b.create_date, b.update_date
...
create table stuinfo (
	stunum number(4) primary key, --중복불가 유니크
	name VARCHAR2(50),
	grade number,
	email VARCHAR2(50),
	phone VARCHAR2(50),
...
--내가한거
select employee_id, emp_name, salary, a.department_id, b.department_name, b.parent_id, b.manager_id, b.create_date, b.update_date, job_id
from employees a,departments b
...
--동일방법
select employee_id, emp_name, salary, a.department_id, job_id, b.department_name, b.create_date, b.update_date
from employees a, departments b
where a.department_id=b.department_id
and a.department_id = (select department_id from employees where emp_name='David Austin');
...
--2개이상 동일한 컬럼을 가지고 있으면 무조건 임의명의 정해 줄 것!!!
select a.department_id, a.employee_id, a.emp_name, a.salary,  a.job_id, min_salary, c.department_name
from  employees a, jobs b, departments c
where a.job_id= b.job_id
and a.department_id = c.department_id
and a.department_id >= 50
and min_salary >= 5000 order by min_salary asc;

select employee_id, emp_name, job_id, department_id, b.department_name
from  employees a, departments b
where job_id = 'sa_man';
...
update salgrade set low_sal = 30000 where grade= 'top';
...
--non-equi 조인트: 동일컬럼이 없을 시에 사용
select employee_id, emp_name, salary, grade 
from employees a, salgrade b
where salary between low_sal and high_sal order by employee_id asc;
...
select a.stunum, a.name, a.grade , b.total, b.avg, c.grade as "학년"
...
case
when salary<=4000 then 'YES'
else 'NO'
end as 연봉협상 from employees;

select * from employees;
select * from kor_loan_status;

select a.department_id, sum(salary) as "총월급", department_name
from employees a, departments b
where a.department_id = b.department_id
group by a.department_id, department_name;

select sum(loan_jan_amt), region
...
select substr(period,1,4) as s_perod, sum(loan_jan_amt)
from kor_loan_status
group by substr(period,1,4);

select substr(period,5,2) as s_perod, sum(loan_jan_amt)
from kor_loan_status
...
select a.employee_id, a.emp_name, a.manager_id, b.emp_name, b.job_id as "직급", c.job_id as"직급2", c.job_title
from employees a, employees b, jobs c
where a.manager_id = b.employee_id --self조인트
and b.job_id=c.job_id; --equi조인트
...
update salary a set salary = (
case
when salary>=14001 then 'top'
when salary>=10001 then '1cls'
when salary>=8001 then '2cls'
when salary>=4001 then '3cls'
when salary>=3001 then '4cls'
when salary<3000 then '5cls'
